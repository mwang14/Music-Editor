...
public interface CompositionBuilder<T> {
  ...
  CompositionBuilder<T> addRepeat(Integer[] repeat);

  CompositionBuilder<T> setStart(int start);

  CompositionBuilder<T> setNumEndings(int numEndings);

  CompositionBuilder<T> setEndStartTime(int endstarttime);

  CompositionBuilder<T> addEndingTime(ArrayList<Integer> endingTime);
